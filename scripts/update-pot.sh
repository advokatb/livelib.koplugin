#!/bin/bash
# Regenerate locale/livelib.pot from _("...") / _[[...]] in plugin Lua sources.
#
...
#   bash scripts/update-pot.sh           # update livelib.pot only
...
POT_FILE="$LOCALE_DIR/livelib.pot"
FILE_LIST="$(mktemp)"
...
# Collect translatable Lua sources (skip secrets and examples)
find "$PLUGIN_DIR" -name '*.lua' \
    ! -path '*/livelib_config.lua' \
    ! -name '*_example.lua' \
...
echo "Done. Translators: copy locale/livelib.pot to locale/<lang>.po and fill msgstr."
